...
def send_web(port, value):
    send_config(port, 'WEB_CONFIG', value, b'WEB_CONFIG_SAVED', (b'WEB_CONFIG_REJECTED', b'CONFIG_REJECTED'))
    print('WebAdmin-Passwort gespeichert. Benutzer admin; HTTP-Adresse steht am Display unter Webzugang.')
...
def main():
    ...
    mode.add_argument("--web-only", action="store_true", help="Nur WebAdmin-Passwort setzen; Firmware 0.9.0 oder neuer")
    ...
    token = getpass.getpass("Panel-Token, kein PRTG-Key (verdeckt, leer = später): ").strip()
    ntp = input("NTP-Server [pool.ntp.org]: ").strip() or "pool.ntp.org"
    if not (1 <= len(ssid.encode()) <= 32 and 8 <= len(password.encode()) <= 63
            and (not token or 16 <= len(token) <= 256) and all(33 <= ord(c) <= 126 for c in token)
    ...
    panel = panel_settings()
    ota = ota_settings()
    choice = input('WebAdmin-Passwort setzen? [1] ja, [0] unverändert [1]: ').strip() or '1'
    if choice not in ('0', '1'):
        raise ValueError('Ungültige Auswahl; nichts übertragen.')
    web = web_settings() if choice == '1' else None
    with serial.Serial(args.port, 115200, timeout=1) as port:
        if web is not None:
            send_web(port, web)
