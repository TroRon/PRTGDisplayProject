...
function pack(changes={}){
 const image=Buffer.alloc(2048,0);image[0]=0xe9;image.writeUInt16LE(9,12);image.writeUInt32LE(0xabcd5432,32);image.write('0.6.1',48);image.write('eaglenet_lcd5b',80);
...
test('OTA routes separate admin/panel access, atomic publish, health remains available',async()=>{
 ...
 try{
  ...
  assert.equal((await request('/api/v1/firmware/upload',adminToken,'PUT',pack())).status,409);
