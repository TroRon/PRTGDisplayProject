# Firmware 0.9.0 – WLAN-Auswahl, Einrichtungshotspot und WebAdmin

**Release 0.9.0.** Als USB-Paket und signiertes OTA-Angebot veröffentlicht. Lokale Tests ersetzen den Test auf dem echten Waveshare LCD-5B nicht. Die bestehenden Releases einschliesslich 0.8.1 bleiben unverändert.
...
4. Benutzer **`admin`**. Bei einem neuen Gerät ohne WebAdmin-Passwort gilt zunächst das angezeigte Hotspot-Passwort auch für WebAdmin; es wird als Passwort-Hash gespeichert. Bei einem bereits eingerichteten Gerät das bisherige WebAdmin-Passwort verwenden. Die Anzeige unterscheidet beide Fälle.
...
Der Hotspot erlaubt höchstens zwei Geräte. Er ist ein lokaler Einrichtungszugang, kein Router/NAT und keine Internetfreigabe. Beim erstmaligen Aktivieren wird ein neues zufälliges Passwort eingerichtet, niemals `admin/admin` oder ein gemeinsames Standardpasswort.

## WebAdmin im normalen WLAN

Unter **Einstellungen → Webzugang** ein Passwort mit 12–63 UTF-8-Bytes zweimal eingeben und speichern. Alternativ per USB einrichten. Solange kein Passwort gesetzt ist, startet kein HTTP-Server. Die Adresse und der Benutzername `admin` werden auf dem Display angezeigt.
