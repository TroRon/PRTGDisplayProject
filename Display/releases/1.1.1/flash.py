...
def main():
    ...
    setup = input("USB-Einrichtung: [0] später/Hotspot, [1] vollständig (WLAN/Panel/OTA/WebAdmin), [2] nur OTA, [3] nur Panel, [4] nur WebAdmin [0]: ").strip().lower()
    if setup in ("1", "2", "3", "4", "j", "ja"):
        input("Warten, bis die Oberfläche sichtbar ist; dann Enter drücken.")
        command = [sys.executable, str(folder / "provision.py"), "--port", port]
        if setup == "2": command.append("--ota-only")
        if setup == "3": command.append("--panel-only")
        if setup == "4": command.append("--web-only")
        subprocess.run(command, cwd=folder, check=True)
